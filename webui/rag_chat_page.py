...
def graph_response(graph, input):
    ...
    for event in graph.invoke(
        ...
        # 如果事件是工具消息
            ...
            with status_placeholder.status("正在查询...", expanded=True) as s:
                ...
                if len(st.session_state["rag_tool_calls"]):
                    if "content" not in st.session_state["rag_tool_calls"][-1]:
                        continue_save = True
                # 显示知识库检索结果
                st.write("知识库检索结果：")
            ...
            # 尝试解析JSON，失败则保存原始内容
            try:
                parsed_content = json.loads(event[0].content)
            except Exception as e:
                st.warning(f"解析工具消息内容失败: {e}")
                parsed_content = event[0].content

            # 保存状态和内容
            if continue_save:
                st.session_state["rag_tool_calls"][-1]["status"] = "已完成知识库检索！"
                st.session_state["rag_tool_calls"][-1]["content"] = parsed_content
            else:
                st.session_state["rag_tool_calls"].append(
                    {
                        "status": "已完成知识库检索！",
        ...
        # 这个打印可以留，也可以注释掉，方便调试
        #print(f"event content (repr): {repr(event[0].content)}")
